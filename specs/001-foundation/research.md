...
| `project:init` end-to-end with Plane + Wiki.js skipped | < 10 s | `tests/integration/test_project_init_wizard.py` |
...
## 8. `project:init` connectivity-check design

**Decision**: For each configured external tool, `project:init` makes ONE
...
and surfaced in `project:init --status`.
...
| Performance | 30s install / 2s lint / 15s cold start / 60s grill / 10s project:init | `tests/perf/` |
...
| `project:init` probe | Read-only call through runtime tier | `project_init/wizard.py` |
