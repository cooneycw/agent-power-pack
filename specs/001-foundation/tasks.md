...
- `src/agent_power_pack/` — installable Python package (linter, generator, cpp:init, grill, secrets, cicd)
...
## Phase 8: User Story 6 — Plane + Wiki.js defaults, `cpp:init` guided wizard (Priority: P2)

**Goal**: `cpp:init` bootstraps a new project end-to-end, including a guided Plane + Wiki.js configuration through the tiered secrets layer. `/issue:*` defaults to Plane, `/docs:c4` publishes to Wiki.js, `/docs:pptx` does not exist.

**Independent Test**: Run `cpp:init` in a fresh directory with valid Plane/Wiki.js creds; both connectivity checks pass and `AGENTS.md` gains an `External Systems` section.
...
- [ ] T084 [US6] Implement `cpp:init` wizard in `src/agent_power_pack/cpp_init/wizard.py` scaffolding AGENTS.md + Makefile + compose.yaml from `src/agent_power_pack/cpp_init/templates/`, then running the Plane + Wiki.js guided config.
- [ ] T085 [US6] Implement the Plane connectivity probe in `src/agent_power_pack/cpp_init/probes.py` (`GET /api/v1/workspaces/{slug}/` expecting 200) per `research.md §8`.
...
- [ ] T088 [US6] Implement the "External Systems" section writer in `src/agent_power_pack/cpp_init/agents_md_update.py` adding a generated section to AGENTS.md with configured endpoints.
...
- [ ] T093 [P] [US6] Integration test in `tests/integration/test_cpp_init_wizard.py` covering the happy path and both `--skip` paths.
- [ ] T094 [P] [US6] Performance test in `tests/perf/test_cpp_init.py` asserting cpp:init (Plane + Wiki.js skipped) completes in < 10s.
...
- [ ] T110 [P] Author `manifests/project/*.yaml` for `project:init` (alias for `cpp:init`), `project:lite`, `project:next`.
...
5. **Defaults release**: add Phase 8 (US6) — Plane/Wiki.js defaults + cpp:init wizard.
